...
int i ;
 long my_id = (long) tid ;
 for ( i = 0; i < TCOUNT; i++){
 pthread_mutex_lock(&count_mutex ) ; //To be removed
 ...
 printf("Just sent sig nal \n" );
 }
 
 

printf("inc_count ( ) : thread %ld , count = %d,uncloking mutex\n" , my_id, count ) ;
 pthread_mutex_unlock(&count_mutex ) ;//To be removed
 sleep ( 1 );
 }
 ...
 printf("watch_count ( ) : thread %ld . Condition sig nal received .Count = %d\n" , my_id, count ) ;
 printf("watch_count ( ) : thread %ldUpdating the count value...\n" , my_id ) ;
 count += 80;
 printf("watch_count ( ) : thread %ldcount now = %d\n" , my_id, count ) ;
 }
 ...
 printf("main : fi ni s h , fi n al count =%d\n" , count ) ;
