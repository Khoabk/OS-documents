...
void * best_fit_allocator(unsigned int size) {
	...
	struct mem_region * best_fit=NULL;
	...
	do{
		...
		if(size <= current_region->size)
		{
			...
			if(best_fit==NULL)best_fit=current_region;
			
			else if( best_fit->size > current_region->size)
			{
				best_fit=current_region;
			}
		}
	...
	if(best_fit!=NULL){
	printf("mem region : %ld  |  ",best_fit->size);
	found=1;
	}
