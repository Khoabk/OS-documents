...
void * worst_fit_allocator(unsigned int size) {
	...
	struct mem_region * worst_fit=NULL;
	...
	do{
		...
		if(size <= current_region->size)
		{
			...
			if(worst_fit==NULL)worst_fit=current_region;
			
			else if( worst_fit->size < current_region->size)
			{
				worst_fit=current_region;
			}
		}
	...
	printf("mem region : %ld  |  ",worst_fit->size);
	...
	struct mem_region* tmp =
		...
		printf("remaining : %ld  |  ",current_region->size);
